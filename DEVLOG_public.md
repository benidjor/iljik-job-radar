...
> **작성일:** 2026년 4월
> **GitHub:** [benidjor/samjik-job-radar](https://github.com/benidjor/samjik-job-radar)
...
| 리뷰완(팀원A) | checkbox | 팀원A 리뷰 완료 여부 |
| 리뷰완(팀원B) | checkbox | 팀원B 리뷰 완료 여부 |
...
### TS-5. Notion people 컬럼 영문 이름 Discord 멘션 매핑 실패

**증상**
Discord 리뷰 알림에서 특정 팀원의 멘션·이름 모두 미표시.

**원인 (두 단계)**

1. **멘션 매핑 실패** — Notion 마니또 컬럼에서 영문 이름 반환 / `DISCORD_USER_MAP` 키는 한글 이름 → ID 매핑 실패
...
```
# .env — 영문 이름 alias 추가
DISCORD_USER_MAP=한글이름:Discord_ID,English Name:Discord_ID,...
```
